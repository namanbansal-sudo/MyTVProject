...
// Custom hook for TV focus management
const useTVFocus = (focusKey: string) => {
  const [isFocused, setIsFocused] = useState(false);
  const ref = useRef<TouchableOpacityHandle>(null);
  ...
  const getRef = useCallback((): number => {
  ...
  return {
    ...
    onBlur,
    getRef,
    focus: () => ref.current?.setNativeProps({ hasTVPreferredFocus: true })
  };
};
...
// Create refs for all focusable elements
const useTVFocusRef = (focusKey: string, onFocus: () => void = () => {}) => {
  const ref = useRef<TouchableOpacityHandle>(null);
  
  useFocusEffect(
    useCallback(() => {
      const focusListener = (event: { eventType: string; tag: number }) => {
        if (event.eventType === 'focus' && ref.current) {
          const node = findNodeHandle(ref.current);
          if (node === event.tag) {
            onFocus();
          }
      ...
      // @ts-ignore - TVEventControl is not in the types
      TVEventControl.enableTVMenuKey();
      const subscription = TVEventControl.addListener('focus', focusListener);
